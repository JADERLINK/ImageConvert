...
namespace GFDLibrary.Textures.GNF
{
    ...
    public class GNFTexture
    {
        ...
        private void Init( SurfaceFormat format, byte mipMapCount, short width, short height, byte[] data, bool isSwizzled )
        {
            ...
            Version                    = 2;
            //LastMipLevel             = mipMapCount; // não pode ser definido;
        }
        ...
        internal void Read( BinaryReader reader )
        {
            ...
            var dataOffset = reader.BaseStream.Position + contentsSize;
        ...
        internal void Write( BinaryWriter writer )
        {
            ...
            writer.BaseStream.Seek( startPosition + 0x100, SeekOrigin.Begin);
            writer.Write( Data );

            var endPosition = writer.BaseStream.Position;
            var fileSize = writer.BaseStream.Position - startPosition;
            writer.BaseStream.Seek( streamSizePosition, SeekOrigin.Begin);
            writer.Write( ( int ) fileSize );
            writer.BaseStream.Seek( endPosition, SeekOrigin.Begin);
        }

        public void SaveToDDS(string filePath)
        {
        ...
        public void SaveToDDS(Stream writer)
        {
            ...
            switch (SurfaceFormat)
            {
                case SurfaceFormat.BC1:
                    fourCC = DDSPixelFormatFourCC.DXT1;
                    break;
                case SurfaceFormat.BC2:
                    fourCC = DDSPixelFormatFourCC.DXT3;
                    break;
                case SurfaceFormat.BC3:
                    fourCC = DDSPixelFormatFourCC.DXT5;
                    break;
                case SurfaceFormat.BC4:
                    fourCC = DDSPixelFormatFourCC.ATI1;
                    break;
                case SurfaceFormat.BC5:
                    fourCC = DDSPixelFormatFourCC.ATI2N_3Dc;
                    break;
                default:
                    throw new NotSupportedException("Invalid GNF");
            }

            var data = Swizzler.UnSwizzle(Data, Width, Height, SurfaceFormat == SurfaceFormat.BC1 ? 8 : 16, SwizzleType.PS4);

            DDSHeader header = new DDSHeader(Width, Height, fourCC);
            header.Save(writer);

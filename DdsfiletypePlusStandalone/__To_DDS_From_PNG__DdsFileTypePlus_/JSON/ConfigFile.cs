...
namespace __To_DDS_From_PNG__DdsFileTypePlus_.JSON
{
    internal static class ConfigFile
    {
        public static void WriteConfigFile(string filename, Config config) 
        {
            ...
            entry["FileFormat"] = Enum.GetName(config.FileFormat);
        ...
        public static Config ParseConfigFile(string filename) 
        {
            ...
            JObject oDDS = (JObject)o["DDS"];

            config.FileFormat = (DdsFileTypePlus.DdsFileFormat)Enum.Parse(typeof(DdsFileTypePlus.DdsFileFormat), oDDS["FileFormat"].ToString());
